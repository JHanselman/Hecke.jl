...
# get an invariant of weight gcd_w.
  ...
  for j in 1:length(non_zero)
	  if C[j] >= 0
	    gcd_w2_scaling *= w1[non_zero[j]]^C[j]
	    gcd_w1_scaling *= w2[non_zero[j]]^C[j]
	  else
	    gcd_w2_scaling *= w2[non_zero[j]]^(-C[j])
	    gcd_w1_scaling *= w1[non_zero[j]]^(-C[j])
	  end
  end
...
function weighted_reduction(w1::Vector{QQFieldElem}, ws::Vector{Int})
    ...
    for p in primes 
	    while all([valuation(w1_min[k], p) >= ws[k] for k in (1:length(w1_min)) ])
	      w1_min = [ divexact(w1_min[k], p^ws[k]) for k in (1:length(w1_min)) ]
	    end
    end
